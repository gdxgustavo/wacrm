...
export function DealForm({
  ...
  async function handleSave() {
    ...
    if (deal) {
      const { error } = await supabase
        ...
        .eq('id', deal.id);
      if (error) {
        toast.error('Falha ao salvar a oferta');
        setSaving(false);
        return;
      }
    } else {
    ...
    setSaving(false);
    toast.success(deal ? 'Oferta atualizada' : 'Negócio criado');
    onOpenChange(false);
    onSaved();
  }
  ...
  async function handleStatusChange(status: DealStatus) {
    ...
    if (error) {
      toast.error('Falha ao atualizar o status da transação');
      return;
    }
    toast.success(
      status === 'won'
  ...
  async function handleDelete() {
    ...
    setDeleting(false);
    if (error) {
      toast.error('Falha ao excluir a oferta');
      return;
    }
        ...
        <div className="flex h-full flex-col">
          <SheetHeader className="border-border/50 border-b p-4">
            <SheetTitle className="text-popover-foreground">
              {deal ? 'Editar oferta' : 'Novo acordo'}
            </SheetTitle>
          </SheetHeader>
            ...
            {deal &&
              (confirmDelete ? (
                <div className="mt-3 flex items-center justify-between gap-2 rounded-md border border-red-500/30 bg-red-500/10 px-3 py-2 text-xs">
                  <span className="text-red-300">Excluir esta oferta?</span>
