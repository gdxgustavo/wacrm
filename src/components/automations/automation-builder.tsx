...
const STEP_META: Record<AutomationStepType, StepMeta> = {
  ...
  update_contact_field: {
    label: 'Atualizar campo de contato',
    icon: PencilLine,
    border: 'border-l-primary',
  },
  create_deal: {
  ...
  wait: { label: 'Espere', icon: Hourglass, border: 'border-l-border' },
  condition: {
    label: 'Condição (se/outro)',
    icon: GitBranch,
...
const TRIGGER_OPTIONS: {
  ...
  },
  {
    ...
    hint: 'A mensagem contém palavras-chave específicas',
  },
  {
  ...
  },
  {
    ...
    hint: 'Quando atribuído a um agente',
  },
  {
    value: 'tag_added',
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      >
        ...
        {value && !selected && (
          <option value={value}>{value} (marca desconhecida)</option>
        )}
      </select>
    </div>
  );
      ...
      >
        ...
        {current && !hasMatch && (
          <option value={current}>
            {templateName} ({language || 'unknown'}) — não na lista aprovada
          </option>
        )}
      </select>
    </FieldBlock>
    ...
    <div className="space-y-2">
      <div>
        ...
        <Input
          ...
          placeholder="por exemplo preços, solicitação de demonstração, fale com vendas"
          className="bg-muted text-foreground"
        />
      </div>
      <div>
...
function StepEditor({
  ...
  switch (step.step_type) {
      ...
      return (
        <>
          <FieldBlock label="Modo">
            ...
            >
              <option value="round_robin">Rodada</option>
    ...
    case 'create_deal':
      return (
        <>
          <FieldBlock label="Código do funil de vendas">
            <Input
    ...
    case 'send_webhook':
      return (
        <>
          ...
          <FieldBlock label="Modelo de corpo (JSON)">
            <Textarea
    ...
    case 'close_conversation':
      return (
        <p className="text-muted-foreground text-xs">
          Define o status da conversa como “fechada”. Nenhuma configuração
          necessária.
        </p>
      );
    default:
