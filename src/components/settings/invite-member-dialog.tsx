...
const ROLE_DESCRIPTIONS: Record<InviteRole, string> = {
  ...
  agent:
    'Pode usar a caixa de entrada, contatos, transmissões, automações e fluxos. Sem configurações ou acesso de membro.',
  viewer:
    'Acesso somente leitura em todas as páginas. Não é possível enviar ou editar nada.',
};
...
export function InviteMemberDialog({
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setResult({
        ...
        accountName: account?.name ?? 'nossa conta do KenzyCRM',
      });
      onCreated();
    } catch (err) {
      console.error('[InviteMemberDialog] erro de criação:', err);
  ...
  function whatsappShareUrl(url: string): string {
    ...
    // for users in multi-team contexts where "our KenzyCRM account"
    // wouldn't be enough to disambiguate.
    const accountName = result?.accountName ?? 'nossa conta do KenzyCRM';
    const message = `Entre em ${accountName} no KenzyCRM usando este link (válido por ${result?.expiresInDays} dias): ${url}`;
          ...
          <>
            <DialogHeader>
              ...
              <DialogDescription className="text-muted-foreground">
                Compartilhe este link com seu novo companheiro de equipe. Eles
                poderão se inscrever (ou fazer login) e ingressar na conta como{' '}
            ...
            <div className="space-y-3 py-2">
              ...
              <div className="rounded-md border border-amber-500/50 bg-amber-500/15 px-3 py-2 text-xs text-amber-200">
                ...
                Nunca armazenamos o texto simples - depois que você fecha esta
                caixa de diálogo, o URL desaparece. Para compartilhar novamente,
          ...
          <>
            <DialogHeader>
              <DialogTitle className="text-popover-foreground">
                Convide um colega de equipe
              </DialogTitle>
              <DialogDescription className="text-muted-foreground">
                Gere um link de convite único. Compartilhe via WhatsApp, Slack
                ou qualquer canal de sua preferência – sem necessidade de
                serviço de e-mail.
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="space-y-4 py-2">
              ...
              <div className="space-y-2">
                ...
                <Input
                  placeholder="por exemplo Sara — equipe de suporte"
                  value={label}
                ...
                />
                <p className="text-muted-foreground text-xs">
                  Ajuda você a lembrar para quem você enviou o link na lista de
                  pendências abaixo.
                </p>
              </div>
            </div>
