...
const STATUS_OPTIONS: {
  ...
  { label: 'Abrir', value: 'open', color: 'text-primary' },
...
export function MessageThread({
  ...
  const sessionInfo = useMemo(() => {
    ...
    if (!lastCustomerMsg)
      return { expired: true, remaining: 'Nenhuma mensagem de cliente' };
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (cancelled) return;
      if (error) {
        console.error('Falha ao buscar reações:', error);
        return;
      }
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ error }) => {
        if (error) console.error('Falha ao redefinir unread_count:', error);
      });
    ...
    async (payload: SendMediaPayload) => {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          console.error('Falha ao enviar mídia:', reason);
          toast.error(`Failed to send: ${reason}`);
          onUpdateMessage(tempId, { status: 'failed' });
  ...
  const handleSendTemplate = useCallback(
    async (
      ...
      try {
        ...
        if (!res.ok) {
          const reason = payload?.error || `HTTP ${res.status}`;
          console.error('Falha ao enviar modelo:', reason);
        ...
        console.error('Falha ao enviar modelo:', err);
        const reason = err instanceof Error ? err.message : 'network error';
        toast.error(`Failed to send template: ${reason}`);
        onUpdateMessage(tempId, { status: 'failed' });
      }
        ...
        <div className="flex items-center gap-2">
          ...
          {onToggleContactPanel && (
            <button
              ...
              aria-label={
                contactPanelOpen
                  ? 'Ocultar painel de contato'
                  : 'Mostrar painel de contato'
              }
