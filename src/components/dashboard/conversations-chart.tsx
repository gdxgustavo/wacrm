    ...
    <section className="border-border bg-card flex h-full flex-col rounded-xl border">
      ...
      <div className="p-5">
        {loading || !data ? (
          ...
          <EmptyState
            icon={MessageSquare}
            title="Nenhuma atividade de mensagem neste intervalo"
      ...
      <footer className="border-border text-muted-foreground flex items-center gap-4 border-t px-5 py-3 text-xs">
        ...
        <LegendDot color="#7c3aed" label="Extrovertido" />
      </footer>
    </section>
  );
}
...
function LineSvg({
  ...
  const incomingPath = data
    .map((p, i) => `${i === 0 ? 'M' : 'eu'}${xFor(i)},${yFor(p.incoming)}`)
    .join(' ');
  const outgoingPath = data
    .map((p, i) => `${i === 0 ? 'M' : 'eu'}${xFor(i)},${yFor(p.outgoing)}`)
    .join(' ');
