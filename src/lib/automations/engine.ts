...
export async function runAutomationsForTrigger(
  ...
  try {
    ...
    if (input.contactId) {
      ...
      if (ownErr) {
        console.error(
          '[automações] falha na verificação de propriedade do contato:',
          ownErr
        );
        return;
      }
    ...
    if (error) {
      console.error('Falha na busca de [automações]:', error);
      return;
    }
    ...
    for (const automation of automations as Automation[]) {
      ...
      try {
        ...
        console.error('Falha na execução de [automações]:', automation.id, err);
      }
    }
  } catch (err) {
    console.error('Falha no envio de [automações]:', err);
  }
}
...
export async function resumePendingExecution(pending: {
  ...
  if (error || !automation) {
    console.error(
      '[automações] currículo: automação ausente',
  ...
  try {
    ...
    console.error('Falha na retomada de [automações]:', err);
    await markPending(pending.id, 'failed');
  }
}
...
async function executeAutomation(automation: Automation, input: DispatchInput) {
  ...
  if (logErr || !log) {
    console.error('[automações] não podem criar log:', logErr);
    return;
  }
  ...
  if (rpcErr) {
    console.error('[automações] contador de incremento falhou:', rpcErr);
  }
}
...
async function runStep(
  ...
  switch (step.step_type) {
    case 'send_message': {
      ...
      if (!args.contactId)
        throw new Error('o envio de mensagem precisa de um contato');
    ...
    case 'send_template': {
      ...
      if (!args.contactId)
        throw new Error('o envio de modelo precisa de um contato');
      if (!cfg.template_name)
        throw new Error('o envio de modelo precisa do nome do modelo');
      const conversationId = await resolveConversationId(args);
    ...
    case 'add_tag': {
      ...
      if (!args.contactId || !cfg.tag_id)
        throw new Error(
          'adicionar etiqueta precisa de contato e ID da etiqueta'
        );
      await db
        .from('contact_tags')
        .upsert(
    ...
    case 'remove_tag': {
      ...
      const cfg = step.step_config as TagStepConfig;
      if (!args.contactId || !cfg.tag_id)
        throw new Error('remover etiqueta precisa de contato e ID da etiqueta');
      await db
        .from('contact_tags')
        .delete()
    ...
    case 'assign_conversation': {
      ...
      if (!args.contactId)
        throw new Error('atribuir conversa precisa de um contato');
    ...
    case 'update_contact_field': {
      ...
      if (!args.contactId)
        throw new Error('atualizar campo do contato precisa de um contato');
    ...
    case 'create_deal': {
      ...
      if (!cfg.pipeline_id || !cfg.stage_id)
        throw new Error('criar negócio precisa de funil e etapa');
    ...
    case 'send_webhook': {
      ...
      if (!cfg.url) throw new Error('enviar webhook precisa de URL');
    ...
    case 'close_conversation': {
      if (!args.contactId)
        throw new Error('fechar conversa precisa de um contato');
      await db
        .from('conversations')
...
async function resolveConversationId(args: ExecuteArgs): Promise<string> {
  ...
  if (!args.contactId)
    throw new Error('não foi possível localizar a conversa: contato ausente');
  const { data, error } = await supabaseAdmin()
    ...
    .eq('contact_id', args.contactId)
    .maybeSingle();
  if (error) throw new Error(`falha ao localizar conversa: ${error.message}`);
  if (!data?.id) throw new Error('nenhuma conversa encontrada para o contato');
