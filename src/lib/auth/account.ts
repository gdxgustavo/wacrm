...
export function toErrorResponse(err: unknown): NextResponse {
  ...
  console.error('[toErrorResponse] erro não categorizado:', err);
...
export async function getCurrentAccount(): Promise<AccountContext> {
  ...
  if (error) {
    console.error('Erro de busca de perfil [getCurrentAccount]:', error);
  ...
  if (accountErr) {
    console.error('Erro de busca da conta [getCurrentAccount]:', accountErr);
    throw new ForbiddenError('Não foi possível carregar o contexto da conta');
  }
  if (!account) {
