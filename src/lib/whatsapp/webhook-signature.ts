...
export function verifyMetaWebhookSignature(
  ...
  if (!secret) {
    console.error(
      '[webhook] META_APP_SECRET não está definido – rejeitando solicitação.' +
        'Configure o env var (Meta → Configurações do aplicativo → Básico → Segredo do aplicativo)' +
        'para ativar a verificação de assinatura.'
    );
    return false;
  }
