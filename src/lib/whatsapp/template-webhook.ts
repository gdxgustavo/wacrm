...
async function handleStatusUpdate(
  ...
  if (error) {
    console.error(
      'Atualização de status [template-webhook] falhou para meta_template_id',
      metaTemplateId,
      error.message
    );
    return;
...
async function handleQualityUpdate(
  ...
  if (error) {
    console.error(
      'Falha na atualização de qualidade de [template-webhook] para meta_template_id',
      metaTemplateId,
      error.message
    );
  }
...
function handleComponentsUpdate(value: TemplateComponentsUpdateValue): void {
  console.info(
    ...
    value.message_template_name,
    '— execute "Sincronizar com a Meta" em Configurações para obter os novos componentes.'
  );
}
