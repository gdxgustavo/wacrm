...
async function loadActiveRunForContact(
  ...
  const { data, error } = await db
    ...
    .limit(1);
  if (error) {
    console.error('[fluxos] erro loadActiveRunForContact:', error.message);
    return null;
  }
...
async function loadFlow(
  ...
  const { data, error } = await db
    ...
    .maybeSingle();
  if (error) {
    console.error('[fluxos] erro loadFlow:', error.message);
    return null;
  }
...
async function loadAllNodes(
  ...
  const { data, error } = await db
    ...
    .eq('flow_id', flowId);
  if (error) {
    console.error('[fluxos] erro loadAllNodes:', error.message);
    return new Map();
  }
...
async function logEvent(
  ...
  if (error) {
    ...
    console.error('[fluxos] erro logEvent:', error.message);
  }
}
...
async function advanceCurrentNodeKey(
  ...
  if (error) {
    console.error('[fluxos] erro advanceCurrentNodeKey:', error.message);
    return false;
  }
...
export async function dispatchInboundToFlows(
  ...
  try {
    ...
    console.error(
      '[fluxos] dispatchInboundToFlows lançado:',
...
async function startNewRun(
  ...
  if (insErr) {
    ...
    console.error('[fluxos] erro de inserção startNewRun:', insErr.message);
    return { consumed: false, outcome: 'no_match' };
  }
  ...
  if (incErr) {
    ...
    console.error('[fluxos] erro de RPC de execução_count:', incErr.message);
  }
