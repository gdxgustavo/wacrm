  ...
  for (const n of nodes) {
    if (seen.has(n.node_key)) {
      issues.push({
        ...
        scope: 'node',
        node_key: n.node_key,
        message: `A chave de nó "${n.node_key}" está duplicada.`,
      });
    }
    seen.add(n.node_key);
  }
    ...
    if (!keywords || keywords.length === 0) {
      ...
      if (blanks > 0) {
        issues.push({
          ...
          scope: 'trigger',
          field: 'trigger_config.keywords',
          message: `${blanks} palavra${blanks === 1 ? ' está' : 's estão'} em branco — não corresponderá a nada.`,
        });
      }
    }
  }
    ...
    case 'start': {
      ...
      if (!cfg.next_node_key) {
        ...
        issues.push({
          ...
          node_key: node.node_key,
          field: 'next_node_key',
          message: `O início aponta para o nó inexistente "${cfg.next_node_key}".`,
        });
      }
      break;
    }
    ...
    case 'send_message': {
      ...
      if (!cfg.next_node_key) {
        ...
        issues.push({
          ...
          node_key: node.node_key,
          field: 'next_node_key',
          message: `O envio de mensagem aponta para o nó inexistente "${cfg.next_node_key}".`,
        });
      }
      break;
    }
    ...
    case 'send_media': {
      ...
      if (!cfg.media_url?.trim()) {
        issues.push({
          ...
          field: 'media_url',
          message:
            'O nó Enviar mídia precisa de um arquivo (carregue um antes de ativar).',
        });
      }
      ...
      if (!cfg.next_node_key) {
        issues.push({
          ...
          node_key: node.node_key,
          field: 'next_node_key',
          message: 'O nó Enviar mídia deve apontar para um próximo nó.',
        });
      } else if (!knownKeys.has(cfg.next_node_key)) {
        issues.push({
          severity: 'error',
          ...
          node_key: node.node_key,
          field: 'next_node_key',
          message: `O nó Enviar mídia aponta para o nó inexistente "${cfg.next_node_key}".`,
        });
      }
      break;
    }
        ...
        rows.forEach((row, ri) => {
          ...
          if (!row.reply_id?.trim()) {
            ...
            issues.push({
              ...
              node_key: node.node_key,
              field: `${field}.reply_id`,
              message: `O ID de linha "${row.reply_id}" está duplicado.`,
            });
          }
    ...
    case 'collect_input': {
      ...
      if (!cfg.var_key?.trim()) {
        ...
        issues.push({
          ...
          node_key: node.node_key,
          field: 'var_key',
          message: `A chave "${cfg.var_key}" deve conter letras, números ou sublinhado e começar com letra ou sublinhado.`,
        });
      }
      if (!cfg.next_node_key) {
        issues.push({
        ...
        issues.push({
          ...
          node_key: node.node_key,
          field: 'next_node_key',
          message: `A coleta de entrada aponta para o nó inexistente "${cfg.next_node_key}".`,
        });
      }
      break;
    }
    ...
    case 'condition': {
      ...
      if (
        ...
        issues.push({
          ...
          node_key: node.node_key,
          field: 'value',
          message: `O operador "${cfg.operator}" normalmente exige um valor de comparação — vazio só corresponderá a valores vazios.`,
        });
      }
      for (const branch of ['true_next', 'false_next'] as const) {
        ...
        if (!key) {
          ...
          issues.push({
            ...
            node_key: node.node_key,
            field: branch,
            message: `O ramo "${branch}" da condição aponta para o nó inexistente "${key}".`,
          });
        }
      }
      break;
    ...
    case 'set_tag': {
      ...
      if (!cfg.mode || !['add', 'remove'].includes(cfg.mode)) {
        issues.push({
          ...
          node_key: node.node_key,
          field: 'mode',
          message: 'Set-tag precisa de um modo (adicionar ou remover).',
        });
      }
      if (!cfg.tag_id) {
        issues.push({
          ...
          node_key: node.node_key,
          field: 'tag_id',
          message: 'Set-tag precisa de uma tag para ser aplicada.',
        });
      }
      if (!cfg.next_node_key) {
        issues.push({
          ...
          node_key: node.node_key,
          field: 'next_node_key',
          message: 'Set-tag deve apontar para um próximo nó.',
        });
      } else if (!knownKeys.has(cfg.next_node_key)) {
        issues.push({
          severity: 'error',
          ...
          node_key: node.node_key,
          field: 'next_node_key',
          message: `A definição de etiqueta aponta para o nó inexistente "${cfg.next_node_key}".`,
        });
      }
      break;
    }
