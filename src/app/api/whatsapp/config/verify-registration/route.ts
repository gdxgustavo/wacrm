...
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      message:
        'O token de acesso armazenado não pode ser descriptografado – provavelmente ENCRYPTION_KEY alterado. Insira novamente o token para reparar.',
    });
  }
  ...
  if (config.waba_id) {
    try {
      ...
      if (!checks.waba_subscribed_to_app) {
        errors.push(
          'WABA não tem aplicativos assinados. Salve novamente a configuração para assinar.'
        );
      }
    } catch (err) {
      errors.push(
    ...
    errors.push(
      'Nenhum ID WABA no arquivo – webhooks não podem ser conectados sem ele. Adicione-o no formulário e salve novamente.'
    );
  }
