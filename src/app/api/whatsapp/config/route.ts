...
export async function GET() {
  try {
    ...
    if (configError) {
      console.error('Erro ao buscar whatsapp_config:', configError);
      return NextResponse.json(
        {
          connected: false,
          reason: 'db_error',
    ...
    try {
      ...
      console.error(
        '[whatsapp/config GET] Falha na descriptografia do token:',
        err
      );
      return NextResponse.json(
        {
          ...
          needs_reset: true,
          message:
            'O token de acesso armazenado não pode ser descriptografado com o ENCRYPTION_KEY atual. Isso geralmente significa que a chave foi alterada ou difere entre os ambientes (local vs Hostinger vs Vercel). Clique em "Redefinir configuração" abaixo e salve novamente.',
        },
        { status: 200 }
      );
    }
    ...
    try {
      ...
      console.error(
        '[whatsapp/config GET] Falha na verificação da Meta API:',
        message
      );
      return NextResponse.json(
        {
    ...
    console.error('Erro na configuração do WhatsApp GET:', error);
    return NextResponse.json(
      {
        connected: false,
        reason: 'unknown',
...
export async function POST(request: Request) {
  try {
    ...
    if (claimedError) {
      console.error(
        'Erro ao verificar a propriedade de phone_number_id:',
        claimedError
      );
      return NextResponse.json(
    ...
    try {
      ...
      const message =
        err instanceof Error ? err.message : 'Erro desconhecido da MetaAPI';
      console.error(
        'A verificação da Meta API falhou durante o salvamento:',
        message
      );
      return NextResponse.json(
    ...
    try {
      ...
      const message =
        err instanceof Error
          ? err.message
          : 'Erro de criptografia desconhecido';
      console.error('Falha na criptografia:', message);
      return NextResponse.json(
        {
          error:
            'Falha ao criptografar o token. Verifique se ENCRYPTION_KEY é uma string hexadecimal válida de 64 caracteres em suas variáveis ​​de ambiente.',
        },
        { status: 500 }
      );
    }
      ...
      if (!pin) {
        ...
        try {
          ...
          registrationError =
            err instanceof Error ? err.message : 'Erro desconhecido da MetaAPI';
          console.error(
            'Número de telefone/registro falhou:',
            registrationError
          );
    ...
    if (waba_id) {
      try {
        ...
        console.warn('WABA subscribed_apps falhou (não fatal):', message);
    ...
    if (existing) {
      ...
      if (updateError) {
        console.error('Erro ao atualizar whatsapp_config:', updateError);
        return NextResponse.json(
      ...
      if (insertError) {
        console.error('Erro ao inserir whatsapp_config:', insertError);
        return NextResponse.json(
    ...
    console.error('Erro no POST de configuração do WhatsApp:', error);
    return NextResponse.json(
...
export async function DELETE() {
  try {
    ...
    if (deleteError) {
      console.error('Erro ao excluir whatsapp_config:', deleteError);
      return NextResponse.json(
    ...
    console.error('Erro na configuração do WhatsApp EXCLUIR:', error);
    return NextResponse.json(
      { error: 'Erro do Servidor Interno' },
      { status: 500 }
    );
