  ...
  try {
    ...
    if (payload.category === 'Authentication') {
      return NextResponse.json(
        {
          error:
            'Os modelos de AUTENTICAÇÃO não são editáveis aqui — gerencie-os no Gerenciador do WhatsApp da Meta.',
        },
        { status: 400 }
      );
    }
    ...
    if (!isDryRun()) {
      ...
      try {
        ...
        const message =
          e instanceof Error ? e.message : 'Falha na edição meta.';
        await supabase
          .from('message_templates')
          .update({
    ...
    if (updErr) {
      return NextResponse.json(
        {
          error: `Editado na Meta, mas não foi possível salvar localmente: ${updErr.message}. Execute "Sincronizar com a Meta" para recuperar.`,
        },
        { status: 500 }
      );
    }
  ...
  try {
    ...
    if (existing.meta_template_id && !isDryRun()) {
      ...
      try {
        ...
        const message =
          e instanceof Error ? e.message : 'Falha na exclusão meta.';
        return NextResponse.json({ error: message }, { status: 502 });
      }
    }
