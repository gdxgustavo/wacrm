...
function normalizeCategory(
  ...
  if (upper === 'UTILIDADE') return 'Utilitário';
  if (upper === 'AUTENTICAÇÃO') return 'Autenticação';
  return 'Marketing';
}
...
function normalizeQualityScore(
  ...
  return upper === 'VERDE' || upper === 'AMARELO' || upper === 'VERMELHO'
    ? (upper as 'VERDE' | 'AMARELO' | 'VERMELHO')
    : null;
}
...
export async function POST() {
  try {
    ...
    for (const t of metaTemplates) {
      const body = (t.components ?? []).find((c) => c.type === 'CORPO');
      const header = (t.components ?? []).find((c) => c.type === 'HEADER');
      const footer = (t.components ?? []).find((c) => c.type === 'RODAPÉ');
      const buttons = (t.components ?? []).find((c) => c.type === 'BOTÕES');
    ...
    console.error('Erro ao sincronizar modelos do WhatsApp:', error);
    return NextResponse.json(
      {
        error:
