  ...
  try {
    ...
    if (payload.category === 'Authentication') {
      return NextResponse.json(
        {
          error:
            'Os modelos de AUTENTICAÇÃO ainda não são suportados aqui — crie-os no Gerenciador do WhatsApp da Meta e use "Sincronizar com a Meta".',
        },
        { status: 400 }
      );
    }
    ...
    try {
      ...
      return NextResponse.json(
        { error: e instanceof Error ? e.message : 'A validação falhou.' },
        { status: 400 }
      );
    }
    ...
    if (dryRun) {
        ...
        return NextResponse.json(
          {
            error:
              e instanceof Error
                ? e.message
                : 'Falha no upload da imagem do cabeçalho.',
          },
          { status: 400 }
        );
      }
      ...
      try {
        ...
        const message = e instanceof Error ? e.message : 'Falha no envio meta.';
        ...
        return NextResponse.json(
          {
            error: isRateLimit
              ? 'Atingimento do limite de metataxa (100 criações de modelos por hora). Tente novamente mais tarde.'
              : message,
          },
    ...
    if (upsertErr) {
      ...
      return NextResponse.json(
        {
          error: `Enviado à Meta, mas não foi possível salvar localmente: ${upsertErr.message}. Execute "Sincronizar com a Meta" para recuperar.`,
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error ? error.message : 'Falha ao enviar modelo.',
      },
      { status: 500 }
    );
  }
