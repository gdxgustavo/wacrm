...
export async function GET(request: Request) {
  try {
    ...
    if (configError || !configs) {
      console.error(
        'Erro ao buscar configurações para verificação:',
        configError
      );
      return NextResponse.json(
    ...
    return NextResponse.json(
      { error: 'Incompatibilidade de token de verificação' },
      { status: 403 }
    );
  } catch (error) {
    console.error('Erro na verificação GET do webhook:', error);
    return NextResponse.json(
...
async function processWebhook(body: { entry?: WhatsAppWebhookEntry[] }) {
  ...
  for (const entry of body.entry) {
    for (const change of entry.changes) {
      ...
      if (configError) {
        console.error(
          'Erro ao buscar whatsapp_config para phone_number_id:',
          phoneNumberId,
          configError
        );
        continue;
      ...
      if (!configRows || configRows.length === 0) {
        console.error(
          'Nenhuma configuração encontrada para phone_number_id:',
          phoneNumberId
        );
        continue;
      }
      ...
      if (configRows.length > 1) {
        console.error(
          ...
          phoneNumberId,
          '– mensagem de entrada descartada. Resolva duplicatas para que cada número seja mapeado para uma única conta.',
...
async function handleStatusUpdate(status: {
  ...
  if (msgErr) {
    console.error('Erro ao atualizar o status da mensagem:', msgErr);
  }
  ...
  if (recFetchErr) {
    console.error('Erro ao buscar o destinatário da transmissão:', recFetchErr);
    return;
  }
  ...
  if (recUpdateErr) {
    console.error(
      'Erro ao atualizar o status do destinatário da transmissão:',
      recUpdateErr
    );
  }
}
...
async function flagBroadcastReplyIfAny(accountId: string, contactId: string) {
  try {
    ...
    if (updErr) {
      console.error(
        'Erro ao marcar o destinatário da transmissão respondeu:',
        updErr
      );
    }
  } catch (err) {
    console.error('flagBroadcastReplyIfAny falhou:', err);
  }
}
...
async function lookupInternalIdByMetaId(
  ...
  if (error) {
    console.error(
      '[webhook] falha na pesquisaInternalIdByMetaId:',
      error.message
    );
    return null;
  }
...
async function handleReaction(
  ...
  if (!reaction.emoji) {
    ...
    if (delError) {
      console.error('Falha na exclusão da reação [webhook]:', delError.message);
    }
    return;
  }
  ...
  if (upsertError) {
    console.error('Falha no upsert de reação [webhook]:', upsertError.message);
  }
}
...
async function processMessage(
  ...
  if (convError) {
    console.error('Erro ao atualizar a conversa:', convError);
  }
  ...
  for (const triggerType of automationTriggers) {
    ...
    }).catch((err) => console.error('Falha no envio de [automações]:', err));
  }
}
