...
export default function PipelinesPage() {
  ...
  const seedDefaultPipeline =
    useCallback(async (): Promise<Pipeline | null> => {
      ...
      if (error || !pipeline) {
        console.error('Falha ao propagar o pipeline:', error?.message);
        return null;
      }
  ...
  const handleDealMoved = useCallback(
    async (dealId: string, newStageId: string) => {
      ...
      const { error } = await supabase
        ...
        .eq('id', dealId);
      if (error) {
        toast.error('Falha ao mover transação');
        refreshDeals();
      }
    },
  ...
  async function handleCreatePipeline() {
    ...
    if (error || !pipeline) {
      toast.error('Falha ao criar o funil de vendas');
      setCreating(false);
      return;
    }
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger className="border-border bg-card text-foreground hover:bg-muted data-[popup-open]:bg-muted inline-flex items-center gap-2 rounded-lg border px-3 py-2 text-sm transition-colors">
              ...
              <span className="font-semibold">
                {selectedPipeline?.name ?? 'Selecione o funil de vendas'}
              </span>
        ...
        <div className="flex items-center gap-2">
          <GatedButton
            ...
            gateReason="criar funis de vendas"
      ...
      {pipelines.length === 0 ? (
        <div className="border-border flex flex-col items-center justify-center rounded-xl border border-dashed py-20">
          ...
          <p className="text-muted-foreground mt-2 text-sm">
            Crie um funil de vendas para começar a acompanhar negócios
          </p>
          <GatedButton
            canAct={canEditSettings}
            gateReason="criar funis de vendas"
        ...
        <DialogContent className="bg-popover border-border sm:max-w-sm">
          ...
          <div className="py-2">
            ...
            <Input
              ...
              placeholder="por exemplo, vendas empresariais"
            ...
            <p className="text-muted-foreground mt-2 text-xs">
              Os estágios padrão (Novo Lead → Ganho) serão criados
              automaticamente.
            </p>
          </div>
