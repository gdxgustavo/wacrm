...
export default function DashboardPage() {
  ...
  const loadAll = useCallback(() => {
    ...
    void loadMetrics(db)
      .then((m) => setMetrics(m))
      .catch((err) => console.error('As métricas do [painel] falharam:', err))
      .finally(() => setMetricsLoading(false));

    void loadConversationsSeries(db, 30)
      .then((s) => setSeries((prev) => ({ ...prev, 30: s })))
      .catch((err) => console.error('A série [painel] falhou:', err))
      .finally(() => setSeriesLoading(false));

    void loadPipelineDonut(db)
      .then((p) => setPipeline(p))
      .catch((err) => console.error('Falha no pipeline do [painel]:', err))
      .finally(() => setPipelineLoading(false));

    void loadResponseTime(db)
      .then((r) => setResponseTime(r))
      .catch((err) =>
        console.error('O tempo de resposta do [painel] falhou:', err)
      )
    ...
    void loadActivity(db, 50)
      ...
      .catch((err) => console.error('Falha na atividade do [painel]:', err))
    ...
    <div className="space-y-5">
      ...
      <div>
        ...
        <p className="text-muted-foreground mt-1 text-sm">
          Análise ao vivo de conversas, contatos, negócios, transmissões e
          automações.
        </p>
      </div>
