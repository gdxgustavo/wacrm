...
export default function BroadcastsPage() {
  ...
  async function fetchBroadcasts() {
    try {
      ...
      setError(
        err instanceof Error ? err.message : 'Falha ao carregar transmissões'
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-foreground text-2xl font-bold">Transmissões</h1>
          <p className="text-muted-foreground mt-1 text-sm">
            Envie mensagens em massa para seus contatos usando modelos
            aprovados.
          </p>
        </div>
        <GatedButton
      ...
      {broadcasts.length === 0 ? (
        <div className="border-border bg-card flex h-64 flex-col items-center justify-center rounded-xl border">
          ...
          <p className="text-foreground text-sm font-medium">
            Ainda não há transmissões
          </p>
          <p className="text-muted-foreground mt-1 text-xs">
            Crie sua primeira transmissão para alcançar seus contatos em grande
            escala.
          </p>
          <GatedButton
            canAct={canCreate}
