...
const FAIL_COPY: Record<PeekFail['reason'], { title: string; body: string }> = {
  not_found: {
    ...
    body: 'Este link não corresponde a um convite válido. Verifique novamente o URL ou peça à pessoa que o convidou para enviar um novo.',
  },
  used: {
  ...
  expired: {
    ...
    body: 'Este convite expirou. Peça ao administrador da conta para enviar uma nova – eles levam alguns segundos para serem gerados.',
  },
  server_error: {
    title: 'Algo deu errado',
    body: 'Não foi possível verificar este convite no momento. Tente atualizar a página daqui a pouco.',
  },
};
...
export default function JoinPage() {
  ...
  const loadPeekAndAuth = useCallback(async () => {
    ...
    try {
      ...
      console.error('[join] erro de espiada:', err);
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 409) {
          setConflictMessage(
            payload.error ||
              'Você já está em outra conta. Faça login com um e-mail diferente para participar deste.'
          );
        } else {
      ...
      console.error('[join] erro de resgate:', err);
  ...
  const handleSignOutAndRetry = useCallback(async () => {
    ...
    try {
      ...
      console.error('[join] erro de saída:', err);
