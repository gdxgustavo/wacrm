...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const fetchProfile = useCallback(async (userId: string) => {
    ...
    try {
      ...
      if (error) {
        console.error('Erro fetchProfile [AuthProvider]:', {
      ...
      if (data) {
        ...
        if (data.account_id) {
          const { data: account, error: accountErr } = await supabase
            ...
            .maybeSingle();
          if (accountErr) {
            console.error('[AuthProvider] erro fetchAccount:', {
      ...
      console.error('[AuthProvider] fetchProfile lançou:', err);
    } finally {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        if (error)
          console.error('Erro de getSession [AuthProvider]:', error.message);
        ...
        console.error('[AuthProvider] init lançou:', err);
      } finally {
